...
def extract_speaker_audio(audio_path, segments, output_dir="speaker_audios/"):
    ...
    for speaker, times in speaker_audio_segments.items():
        ...
        print(f"Saved {output_dir}/{speaker}.wav")
...
def extract_audio_features(audio_paths):   
    ...
    for speaker, path in audio_paths.items():
        ...
        # Print features for the current speaker
        print(f"\nFeatures for {speaker}:")
        for feature_name, value in features[speaker].items():
            print(f"  {feature_name}: {value:.4f}")  # Print with 4 decimal precision
        print("-" * 40)
...
def analyze_voice_emotion(audio_paths, chunk_duration_ms=30000):
    ...
    for speaker, path in audio_paths.items():
        ...
        for i in range(num_chunks + 1):
            ...
            result = emotion_model("temp_chunk.wav")
        ...
        print(f"\nEmotion Analysis Results for {speaker} (Averaged):")
        for emotion, avg_score in averaged_results.items():
            print(f"Emotion: {emotion} | Average Confidence: {avg_score:.4f}")
        print("-" * 40)
