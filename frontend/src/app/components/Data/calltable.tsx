        ...
        const calls: Call[] = filtered.map((call: ApiCall) => ({
          ...
          transcript: typeof call.transcript?.text === "string"
            ? call.transcript.text.split("\n\n").map((block) => {
                ...
                const speaker = speakerRaw === "AGENT"
            ...
            {selectedCall.transcript.length > 0 ? (
              <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
                {selectedCall.transcript.map((entry, index) => (
                  <div key={index} className={`flex ${entry.speaker !== selectedCall.name ? "justify-start" : "justify-end"}`}>
                    <div className={`px-4 py-2 rounded-lg max-w-[75%] shadow ${entry.speaker.toLowerCase().includes("client") ? "bg-gray-700 text-white" : "bg-blue-600 text-white"}`}>
        ...
        >
          ...
          >
            ...
            {selectedCall.report?.silence_percentage !== undefined && (
              <p><strong>Silence %:</strong> {safeFixed(selectedCall.report?.silence_percentage)}%</p>
            )}

            {Array.isArray(selectedCall.report?.speakers) && selectedCall.report.speakers.length > 0 && (
              <div className="mt-4">
                ...
                {selectedCall.report.speakers.map((speaker, index) => (
                  <div key={index} className="mb-2 border-t border-gray-700 pt-2">
                    ...
                    <p className="text-sm">
                      ...
                      Ira: {safeFixed(speaker.emotions.anger)} |
                      Neutralidad: {safeFixed(speaker.emotions.neutrality)}<br />
                      Sentimiento: {speaker.emotions.text_sentiment} ({safeFixed(speaker.emotions.text_sentiment_score)})
                    </p>
                    <p className="text-sm mt-1">
                      <strong>Voz:</strong><br />
                      Pitch: {safeFixed(speaker.voice.pitch)} Hz |
                      Tempo: {safeFixed(speaker.voice.tempo)} BPM<br />
                      Volumen: {safeFixed(speaker.voice.loudness, 4)} |
                      ZCR: {safeFixed(speaker.voice.zcr, 4)} |
                      HNR: {safeFixed(speaker.voice.hnr)}
                    </p>
                  </div>
                ))}
              </div>
            ...
            {Array.isArray(selectedCall.report?.suggestions) && selectedCall.report.suggestions.length > 0 && (
              <div className="mt-4">
